...
# Monkey-patch for modelscope compatibility
if not hasattr(np, 'float'):
    np.float = float
try:
    import fairseq.utils as _fu
    # 如果已经有 index_put，就跳过
    if not hasattr(_fu, 'index_put'):
        import torch
        def index_put(src, indices, values, accumulate=False):
            # src: Tensor
            # indices: tuple of index Tensors
            # values: Tensor
            # accumulate: bool
            return src.index_put(indices, values, accumulate)
        _fu.index_put = index_put
        print('[patch] fairseq.utils.index_put added')
except ImportError:
    # fairseq 不存在就不管
    pass
def parse_args():
    ...
    parser.add_argument(
        "--csv",
        type=str,
        default='/home/jiaji_lu/AR/Infinity/evaluation/DPG/dpg_bench_updated.csv',
    )
    parser.add_argument(
        "--res-path",
        type=str,
...
def prepare_dpg_data(args):
    ...
    for i, line in data.iterrows():
        ...
        try:
            ...
            qid = 0
        dep = line.dependency
        if not isinstance(dep, str) or dep.strip() == "" or dep != dep:  # dep!=dep 用来检测 NaN
            dependency_list_str = ["0"]
        else:
            dependency_list_str = dep.split(',')
        dependency_list_int = []
        ...
        qnl = line.question_natural_language
        if not isinstance(qnl, str) or qnl != qnl:
            qnl = ""
        category = qnl.split('(')[0].strip()
